...
function BtnSet_fn() {
	...
	$saveBtn.click(function () {//확인 버튼 클릭시
		...
		saveQuiz_fn(qId, qType, JSON.stringify(latexStr_array), ansStr);
		$(".btn.btn-retry").show();
		$(".btn.btn-solve").show();
		$(this).hide();
		$('.btn').addClass('reload');
...
function getQData_fn(data) {//처음 페이지 진입시 데이타 받아오는 함수
	if (!isEmpty(data.input)) {
		...
		for (i = 0; i < ans_array.length; i++) {
			...
			viewMathJaxStr(ans_array[i], $("#QuizInput" + (i + 1)))
		}
		chkDap_fn();
		$(".btn.btn-solve").show();
		$(".btn.btn-retry").show();
