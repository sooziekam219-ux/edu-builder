...
async function insertLatex_fn(l_str) {//라텍스 구문의 버튼 클릭시 라텍스 구문문자열을 바로 받아 QuizInput에 라텍스 구문을 추가
    try {
        ...
        l_str = l_str.replace(/\\sim\b/g, '\\text{~}');//~ 물결표시도 닮음 표시로 해 달라고 요청함.
        l_str = l_str.replace(/\\parallel\b/g, '\\text{//}');

        // 앞뒤 공백 및 줄바꿈 제거
        l_str = l_str.trim().replace(/\r?\n/g, '');

        // 두 줄 이상인지 확인 (cases, array 등 포함)
        const isMultiLine = /\\\\/.test(l_str);

        // 클래스명 설정
        const className = isMultiLine ? 'mathjax-rendered doubleline' : 'mathjax-rendered';

        // 수식 삽입: 다중행 수식에는 &nbsp;를 붙이지 않음
        const latexSpan = `<span class="${className}" contenteditable="false">${l_str}</span>`;
        const latexStr = isMultiLine ? latexSpan : `${latexSpan}&nbsp;`;
        document.execCommand('insertHTML', false, latexStr);
