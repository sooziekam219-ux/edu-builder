...
/* 2차 추가 API 연동 화면 조작 스크립트 */
if (typeof AIDT_CURRENT_INDEX === 'undefined') {
    let AIDT_CURRENT_INDEX = 0;
}
// 페이지 이동 함수
function moveToPage(index) {
    if (index >= 0 && index < view_pageinfo.length) {
        AIDT_CURRENT_INDEX = index;
        window.location.href = `${view_pageinfo[index].url}`;
    }
}

// 이전 페이지 이동
function prevPage() {
    if (AIDT_CURRENT_INDEX > 0) {
        moveToPage(AIDT_CURRENT_INDEX - 1);
    } else {
        alert("첫 페이지입니다.");
    }
}
...
// 다음 페이지 이동
function nextPage() {
    if (AIDT_CURRENT_INDEX < view_pageinfo.length - 1) {
        moveToPage(AIDT_CURRENT_INDEX + 1);
    } else {
        alert("마지막 페이지입니다.");
    }
