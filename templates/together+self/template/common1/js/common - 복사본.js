...
function btnSet_fn() {
    ...
    if ($(".btn-math").length > 0) {//수식 입력기
        ...
        $(".btn-math").click(function () {
            if (quiz_on == false) return;
            call_EXPRESS($(".btn-math").index(this) + 1);//한페이지에 여러 수식 입력기가 있는 경우 인덱스 값을 넘김
    ...
    if ($(".btn.btn-solve").length > 0) {//풀이 버튼이 있는 경우
        $(".btn.btn-solve").on("click", function () {
            $(this).addClass("on");
            $('.pop.solution').addClass('show');
            $('.pop.solution [role="document"]').attr("tabindex", 0).focus();
        });
    ...
    try {
        if ($(".pop.solution").length > 0 && (typeof (popDraggable) == "undefined" || popDraggable)) {//풀이가 있는 경우 - act내에서 popDraggable=true 또는 따로 선언안되어 있으면 드래그 가능
            if (detectDevice() != "chromebook" && detectDevice() != "windows tablet" && detectDevice() != "tablet") {//chromebook, windows tablet 이 아닌 경우 드래그 가능
                var popDiv = $(".pop.solution");
                var div0 = $(".pop.solution div").eq(0);
                popDiv.css("pointer-events", "none");
...
function hideBtnMath_fn() { if ($(".btn-math").length > 0) $(".btn-math").css("background-image", "none"); }//수식입력기 버튼을 숨김 처리
function showBtnMath_fn() { if ($(".btn-math").length > 0) $(".btn-math").css("background-image", btnMathBgSrc); }//수식입력기 버튼을 보임 처리
...
function correctSet_fn(dap_arr) {//dap_array 내용을 correct 클래스에 순서대로 Mathml 형태로 변환해서 보여주는 함수
...
function saveQuiz_fn(q_id, q_type, q_val, ip_val) {
    ...
    if (qCorrectType_bool) {//
        ...
        if (ip_val == "") o_str = " ";
        QuizCorrectSave_fn(qId, qType, o_str);
